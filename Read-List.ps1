...
param(
	...
	begin 
	{
			...
			{
				 	Write-Host ""
					Write-Host "Read-List.ps1	Created by  August 2014 " -ForegroundColor Cyan  -backgroundcolor  DarkGray	
					#Write-Host ""
			}
	...
	process 
	{
		...
		$global:gAllItemsTrim = $AllItemsTrim
		
			if(!$keep_order)
			{
				$AllItemsTrim  = @( $AllItemsTrim  | sort $ItemsFromCSVFileProperties.name)
			}
			
			$global:gAllItemsTrimSorted = $AllItemsTrim
