...
Write-Host ""
Write-Host "sleeptimer.ps1;	Created by " -ForegroundColor Cyan  -backgroundcolor  DarkGray	
#Write-Host ""	
...
function  Timecounter ($timecounter )
{
	...
	{	
		...
		$timeLeft = $ts - $CurrentTime 

		write-host "$([string]::Format("`r{0:d2}:{1:d2}:{2:d2} Left / {3:d2}:{4:d2}:{5:d2} Elapsed / $requestedtimedisplay Requested [hr:min:sec] ",  $timeLeft.hours , $timeLeft.minutes ,  $timeLeft.seconds ,$CurrentTime.hours, $CurrentTime.minutes,  $CurrentTime.seconds   ) ) "  -nonewline -ForegroundColor Cyan	-BackgroundColor	Blue
